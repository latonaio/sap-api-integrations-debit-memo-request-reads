...
func (c *SAPAPICaller) callDebitMemoRequestSrvAPIRequirementHeader(api, debitMemoRequest string) ([]sap_api_output_formatter.Header, error) {
	...
	param := c.getQueryWithHeader(map[string]string{}, debitMemoRequest)
...
func (c *SAPAPICaller) callDebitMemoRequestSrvAPIRequirementItem(api, debitMemoRequest, debitMemoRequestItem string) ([]sap_api_output_formatter.Item, error) {
	...
	param := c.getQueryWithItem(map[string]string{}, debitMemoRequest, debitMemoRequestItem)
...
func (c *SAPAPICaller) getQueryWithHeader(params map[string]string, debitMemoRequest string) map[string]string {
	if len(params) == 0 {
		params = make(map[string]string, 1)
	}
	params["$filter"] = fmt.Sprintf("DebitMemoRequest eq '%s'", debitMemoRequest)
	return params
}

func (c *SAPAPICaller) getQueryWithItem(params map[string]string, debitMemoRequest, debitMemoRequestItem string) map[string]string {
	if len(params) == 0 {
		params = make(map[string]string, 1)
	}
	params["$filter"] = fmt.Sprintf("DebitMemoRequest eq '%s' and DebitMemoRequestItem eq '%s'", debitMemoRequest, debitMemoRequestItem)
	return params
}
